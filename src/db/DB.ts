...
export const getUserScore = async (id: string): Promise<number> => {
  ...
  const highscore = await db.tables.Highscore.single((c) => c.score).where(
    (c) => c.equals({ id })
  );
...
export const setUserScore = async (
  ...
  let highscoreTable = await db.tables.Highscore.select().orderBy({
    score: "ASC",
  });
...
export const getHighscores = async (): Promise<Highscore[]> => {
  ...
  return db.tables.Highscore.select((c) => [c.name, c.score]).orderBy({
    score: "ASC",
  });
};
